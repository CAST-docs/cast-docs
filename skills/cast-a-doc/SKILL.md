...
- Do not publish Mermaid, PlantUML, Graphviz, or flowchart source as a `code` block or visible `<pre>` fallback. Use a CAST Docs `diagram` block that renders to inline SVG, includes the renderer-owned diagram viewer hook, and supports zoom/download; a static `data:image/svg+xml` image fallback is not CAST-compliant. Then run both JSON and HTML validation.
